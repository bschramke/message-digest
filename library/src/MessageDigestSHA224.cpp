...
namespace
{
  ...
  inline uint32_t f4(uint32_t a, uint32_t b, uint32_t c)
  {
...
void MessageDigestSHA224::update(const void *data, const size_t offset, const size_t len)
{
...
void MessageDigestSHA224::processBlock(const void *data)
{
  ...
  // extend to 24 words
  for (; i < 24; i++)
    words[i] = words[i-16] + f3(words[i-15],words[i-15],words[i-15]) +
               words[i-7] + f4(words[i- 2],words[i- 2],words[i- 2]);
...
void MessageDigestSHA224::processBuffer()
{
